...
class Template implements RunsOnPluginActivationInterface {
	...
	public static function init_active_template( self $template ) {
		add_action(
			...
			function() use ( $template ) {
				...
				wp_enqueue_style( 'cfw_front_template_css', $template->get_template_uri() . "/style{$min}.css", array(), $template->get_version() );
				wp_enqueue_script( 'wc-checkout', $template->get_template_uri() . "/theme{$min}.js", array( 'jquery' ), $template->get_version(), true );
			}
		);
