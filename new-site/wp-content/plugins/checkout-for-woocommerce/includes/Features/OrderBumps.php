...
class OrderBumps extends FeaturesAbstract {
	...
	protected function run_if_cfw_is_enabled() {
		...
		add_action( 'cfw_checkout_update_order_review', array( $this, 'handle_adding_order_bump' ) );
	...
	public function output_bumps( string $location = 'all', string $container_class = '' ) {
		...
		foreach ( $bumps as $bump ) {
			if ( $bump->is_displayable() && ( 'all' === $location || $bump->get_display_location() === $location ) ) {
	 ...
	 * @throws \Exception
	 */
	public function handle_adding_order_bump( $post_data ) {
	...
	protected function add_bump_to_cart( $bump_id ): bool {
		...
		/**
		 * We're not sure what this solves but we know if solves something.
		 */
	 ...
	 * @throws \Exception
	 */
	function show_bump_discount_on_cart_item( string $price_html, array $cart_item, \WC_Product $product ): string {
		if ( ! isset( $cart_item['_cfw_order_bump_id'] ) ) {
			return $price_html;
		}
