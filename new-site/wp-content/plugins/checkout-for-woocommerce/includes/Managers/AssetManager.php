...
class AssetManager {
	...
	public function set_assets() {
		...
		if ( ! cfw_is_checkout() && ! is_checkout_pay_page() && ! is_order_received_page() ) {
			return;
		}
		...
		$front = trailingslashit( CFW_PATH_ASSETS ) . 'dist';
		...
		$cfw_event_data = apply_filters(
			...
			array(
				...
				'settings'                   => array(
					 ...
					 */
					'enable_checkout_login_reminder'    => apply_filters( 'cfw_suppress_default_login_form', SettingsManager::instance()->get_setting( 'login_style' ) !== 'woocommerce' ) && 'yes' === get_option( 'woocommerce_enable_checkout_login_reminder' ) && cfw_is_checkout(),
