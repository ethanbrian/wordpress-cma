...
function cfw_customer_info_tab_heading() {
	...
	<h3>
		 ...
		 *
		 * @param $customer_info_heading Customer info tab heading
		 * @since 2.0.0
		 */
...
function cfw_customer_info_tab_login() {
	...
	/**
	 * Filters whether to suppress default login form
	 *
	 * @param bool $suppress_default_login_form Suppress default login form
	 * @since 2.0.0
	 */
	$enable_enhanced_login = apply_filters( 'cfw_suppress_default_login_form', SettingsManager::instance()->get_setting( 'login_style' ) !== 'woocommerce' ) && 'yes' === get_option( 'woocommerce_enable_checkout_login_reminder' );
	?>
	...
	<ul class="cfw-review-pane cfw-module">
		...
		<?php if ( WC()->cart->needs_shipping() ) : ?>
			...
			<li class="shipping-method">
				...
				<div role="cell" class="col-2 cfw-review-pane-link">
					<?php if ( SettingsManager::instance()->get_setting( 'skip_shipping_step' ) !== 'yes' ) : ?>
					...
					<div class="col-lg-8 no-gutters">
						...
						$output = cfw_form_field(
							...
							array(
								...
								'start'       => false,
								'end'         => false,
								'wrap'        => FormAugmenter::instance()->input_wrap( 'text', 12, 10 ),
								'return'      => true,
							)
						);
...
function cfw_form_attributes( $id = false, $row = true, $action = true ) {
	...
	?>
	id="<?php echo $id ? $id : 'checkout'; ?>" name="<?php echo $id ? $id : 'checkout'; ?>" class="cfw-customer-info-active <?php echo ( 'order_review' !== $id ) ? 'woocommerce-checkout checkout' : ''; ?> <?php echo $row ? ' row' : ''; ?>" method="POST" formnovalidate="" <?php echo ( $id !== 'order_review' ) ? 'data-parsley-focus="first"': ''; ?> enctype="multipart/form-data" <?php echo $action ? 'action="' . esc_url( wc_get_checkout_url() ) . '"' : ''; ?>
	<?php
