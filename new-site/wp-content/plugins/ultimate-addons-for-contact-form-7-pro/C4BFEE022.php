<?php
if(!class_exists("C4BFEE022")) {
	class C4BFEE022 {
    	public $key = "A225705348F718F2";
    	private $product_id = "2";
    	private $product_base = "ultimate-addons-for-contact-form-7-pro";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_ultimate-addons-for-contact-form-7-pro_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=ultimate-addons-for-contact-form-7-pro_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
