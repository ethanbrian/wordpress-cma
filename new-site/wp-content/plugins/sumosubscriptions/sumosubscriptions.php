...
final class SUMOSubscriptions {
	...
	public function woocommerce_dependency_warning_message() {
		if ( ! $this->is_woocommerce_active() && is_admin() ) {
			echo '<div class="error"><p> SUMO Subscriptions Plugin requires WooCommerce Plugin should be Active !!! </p></div>' ;
		}
		return ;
	}
	...
	private function include_files() {
		...
		include_once('includes/class-subscription-variation-data.php') ;
	...
	public function init_upon_activation() {
		SUMOSubscriptions_Admin_Welcome::load() ;
	}
	...
	/**
	 * Init SUMOSubscriptions when WordPress Initializes. 
	 */
	public function init() {
		...
		include_once('includes/compatibilities/subscription-compatibility-functions.php') ;
		include_once('includes/compatibilities/subscription-deprecated-functions.php') ;

		if ( class_exists( 'TM_Extra_Product_Options' ) || class_exists( 'THEMECOMPLETE_Extra_Product_Options' ) ) {
			include_once('includes/compatibilities/class-wc-tm-extra-product-options.php') ;
		}
	}
