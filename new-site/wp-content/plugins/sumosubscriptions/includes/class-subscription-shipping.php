...
class SUMO_Subscription_Shipping {
	...
	public static function save_shipping_address( $subscriber_id ) {
		...
		$get             = $_GET ;
		$subscription_id = absint( wp_unslash( $get[ 'subscription_id' ] ) ) ;
		$shipping        = array(
			'updated_via'   => $subscription_id,
			'update_to_all' => isset( $_REQUEST[ 'update_to_all' ] ) && 'yes' === wc_clean( wp_unslash( $_REQUEST[ 'update_to_all' ] ) ),
				) ;

		if ( update_user_meta( absint( wp_unslash( $get[ 'subscriber_id' ] ) ), 'sumosubs_shipping_address', $shipping ) ) {
			...
			sumo_add_subscription_note( $note, $subscription_id, 'success', __( 'Shipping Address Changed', 'sumosubscriptions' ) ) ;
		} else {
			sumo_add_subscription_note( __( 'Error updating shipping address.', 'sumosubscriptions' ), $subscription_id, 'failure', __( 'Updating Shipping Address', 'sumosubscriptions' ) ) ;
		}
