...
class SUMO_PayPal_Adaptive_Gateway extends WC_Payment_Gateway {
	...
	public function gateway_deprecated_notice() {
		if ( 'yes' === $this->enabled ) {
			$notice_dismissed = get_user_meta( get_current_user_id(), 'dismissed_sumo_paypal_preapproval_gateway_deprecated_notice', true ) ;

			if ( ! $notice_dismissed ) {
				printf( '<div id="message" class="updated woocommerce-message"><a class="woocommerce-message-close notice-dismiss" href="' . esc_url( wp_nonce_url( add_query_arg( 'wc-hide-notice', 'sumo_paypal_preapproval_gateway_deprecated' ), 'woocommerce_hide_notices_nonce', '_wc_notice_nonce' ) ) . '">%1$s</a><p>%2$s</p></div>', esc_html__( 'Dismiss', 'woocommerce' ), wp_kses_post( __( 'Please note that PayPal is informing to users that they are not accepting requests for Application ID from 1 Dec 2017 onwards. You will not be able to use SUMO Subscriptions - PayPal Adaptive Payment gateway if you don\'t have an Application ID already and this payment method is going to be deprecated in the future versions of the plugin. Instead of this payment method, we recommend you to use Standard PayPal or SUMO Subscriptions - PayPal Reference Transactions payment method which are supported for automatic recurring.', 'sumosubscriptions' ) ) ) ;
			}
		}
	}
