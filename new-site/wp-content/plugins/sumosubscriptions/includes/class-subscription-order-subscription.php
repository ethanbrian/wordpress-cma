	...
	public static function cart_contains_valid_products() {
		...
		switch ( self::$get_option[ 'product_select_type' ] ) {
			case 'included-products':
				$valid    = 0 === count( array_diff( $items_in_cart, self::$get_option[ 'included_products' ] ) ) ? true : false ;
				break ;
			case 'excluded-products':
				$valid    = 0 === count( array_intersect( $items_in_cart, self::$get_option[ 'excluded_products' ] ) ) ? true : false ;
				break ;
			case 'included-categories':
				...
				$valid          = 0 === count( array_diff( $items_in_cart, $found_products ) ) ? true : false ;
				break ;
			case 'excluded-categories':
				...
				$found_products = ! empty( $products->posts ) ? $products->posts : array() ;
				$valid          = 0 === count( array_intersect( $items_in_cart, $found_products ) ) ? true : false ;
				break ;
		}
