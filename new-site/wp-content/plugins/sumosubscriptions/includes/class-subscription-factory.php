 ...
 * 
 * Creating right object for Subscription or Product.
 * 
 * @class SUMO_Subscription_Factory
 */
class SUMO_Subscription_Factory {

	protected static $internal_args = array(
		'susbcription_status'                     => '',
		'trial_selection'                         => '',
		'fee_type'                                => '',
		'trialfee'                                => '',
	...
	public static function get_subscription_id( $subscription ) {
		...
		} elseif ( $subscription instanceof SUMO_Subscription || $subscription instanceof SUMO_Subscription_Product ) {
			return $subscription->get_id() ;
		} elseif ( ! empty( $subscription->ID ) ) {
			return absint( $subscription->ID ) ;
		} elseif ( is_a( $post, 'WP_Post' ) && in_array( get_post_type( $post ), array( 'sumosubscriptions', 'product' ) ) ) {
	 ...
	 * @return array
	 */
	public static function get_subscription( $subscription, $user_id = 0 ) {
		$subscription_meta       = array() ;
		$subscription_id         = 0 ;
		$subscription_product_id = 0 ;
		$the_subscription_id     = self::get_subscription_id( $subscription ) ;
				...
				case 'product_variation':
					...
					if ( sumo_can_user_purchase_as_subscription( $subscription_product_id, $user_id ) ) {
						...
						if ( SUMO_Subscription_Synchronization::$sync_enabled_site_wide ) {
							switch ( $subscription_period ) {
								case 'W':
									...
									break ;
								case 'M':
									$is_synced = $synchronization_period_value > 0 && ( '2' === SUMO_Subscription_Synchronization::$sync_mode || ( '1' === SUMO_Subscription_Synchronization::$sync_mode && ( 0 === 12 % $subscription_period_value || '24' === $subscription_period_value ) ) ) ;

									//BKWD CMPT <= 9.6
									if ( $is_synced && '1' === SUMO_Subscription_Synchronization::$sync_mode && '1' === $subscription_period_value && version_compare( $subscription_version, '9.6', '<=' ) ) {
						...
						$subscription_meta = array(
							...
							'additional_digital_downloads_status'     => sumo_is_additional_digital_downloads_enabled_in_the_site() && 'yes' === get_post_meta( $subscription_product_id, 'sumo_enable_additional_digital_downloads', true ) ? '1' : '2',
