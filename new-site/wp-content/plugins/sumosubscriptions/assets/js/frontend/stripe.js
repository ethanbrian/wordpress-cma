/* global sumosubs_stripe_params */

jQuery( function( $ ) {
	'use strict' ;

	// sumosubs_stripe_params is required to continue, ensure the object exists
	if ( typeof sumosubs_stripe_params === 'undefined' ) {
		return false ;
	}
	...
	var sumo_stripe = {
		...
		isStripeChosen : function() {
			if ( sumosubs_stripe_params.payment_method === $( '.payment_methods input[name="payment_method"]:checked' ).val() ) {
				return true ;
			}
			return false ;
		},
		initElements : function() {
			if ( 'inline_cc_form' === sumosubs_stripe_params.checkoutmode ) {
		...
		throwErr : function( error ) {
			...
			if ( 'email_invalid' === error.code ) {
				message = sumosubs_stripe_params.email_invalid ;
			} else if (
					/*
				...
				message = sumosubs_stripe_params.invalid_request_error ;
			}

			if ( 'card_error' === error.type && sumosubs_stripe_params.hasOwnProperty( error.code ) ) {
				message = sumosubs_stripe_params[ error.code ] ;
			}

			if ( 'validation_error' === error.type && sumosubs_stripe_params.hasOwnProperty( error.code ) ) {
				message = sumosubs_stripe_params[ error.code ] ;
			}
	...
	try {
		...
		sumo_stripe.stripeClient = Stripe( sumosubs_stripe_params.key ) ;
