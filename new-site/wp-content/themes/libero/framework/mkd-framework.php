...
require_once MIKADO_FRAMEWORK_ROOT_DIR."/lib/google-fonts.php";
...
if(!function_exists('libero_mikado_enqueue_admin_scripts')) {
	...
	function libero_mikado_enqueue_admin_scripts() {
		...
		wp_enqueue_media();
		wp_enqueue_script('libero-mikado-dependence');
		wp_enqueue_script('libero-mikado-twitter-connect');
        wp_enqueue_script("libero-mikado-dependence", get_template_directory_uri().'/framework/admin/assets/js/mkd-ui/mkd-dependence.js', array(), false, true);
        wp_enqueue_script("libero-mikado-twitter-connect", get_template_directory_uri().'/framework/admin/assets/js/mkd-twitter-connect.js', array(), false, true);
...
if(!function_exists('libero_mikado_enqueue_nav_menu_script')) {
	...
	function libero_mikado_enqueue_nav_menu_script($hook) {
		if($hook == 'nav-menus.php') {
			wp_enqueue_script('libero-mikado-nav-menu', get_template_directory_uri().'/framework/admin/assets/js/mkd-nav-menu.js');
			wp_enqueue_style('libero-mikado-nav-menu', get_template_directory_uri().'/framework/admin/assets/css/mkd-nav-menu.css');
		}
	}
...
if(!function_exists('libero_mikado_meta_box_add')) {
	...
	function libero_mikado_meta_box_add() {
		...
		foreach ($libero_mikado_Framework->mkdMetaBoxes->metaBoxes as $key=>$box ) {
			...
			if(is_array($box->scope) && count($box->scope)) {
				foreach($box->scope as $screen) {
					libero_mikado_create_meta_box_handler( $box, $key, $screen );
		...
		if ( libero_mikado_is_gutenberg_editor() || libero_mikado_is_gutenberg_plugin() ) {
			libero_mikado_enqueue_meta_box_styles();
			libero_mikado_enqueue_meta_box_scripts();
		} else {
